...
| **1** | generic-api-key      | `backend/scripts/prodExpand.cjs:19` (historical) | **real — historical Alpha Vantage key `***REDACTED_AV_KEY***…` committed in an early revision; current source uses `process.env.ALPHA_VANTAGE_KEY`** |

### Action for the one real finding

- Key fragment: `***REDACTED_AV_KEY***` (Alpha Vantage free-tier)
