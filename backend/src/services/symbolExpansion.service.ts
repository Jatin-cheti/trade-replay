...
export async function ingestGlobalSymbolsIncremental(): Promise<FullExpansionReport> {
  ...
  if (isFmpAvailable()) {
    const [fmpEtfs, fmpTraded, fmpCrypto, fmpForex, fmpCommodities, fmpBonds, fmpEconomy, fmpOptions, fmpFutures] = await Promise.all([
      expandFmpEtfs(),
      expandFmpAvailableTraded(),
      expandFmpCrypto(),
      expandFmpForex(),
    ...
    allResults.push(fmpEtfs, fmpTraded, fmpCrypto, fmpForex, fmpCommodities, fmpBonds, fmpEconomy, fmpOptions, fmpFutures);
  } else {
    allResults.push(
      fmpSkippedResult("fmp-etfs"), fmpSkippedResult("fmp-traded"),
      fmpSkippedResult("fmp-crypto"), fmpSkippedResult("fmp-forex"),
      fmpSkippedResult("fmp-commodities"), fmpSkippedResult("fmp-bonds"),
      fmpSkippedResult("fmp-economy"), fmpSkippedResult("fmp-options"),
      fmpSkippedResult("fmp-futures"),
    );
  }
