 ...
 * =====================
 * Shared factory for all TV capture slot specs.
 * Exports ALL_CAPTURE_TOOLS (27 tools), buildScenarios, and registerCaptureSuite.
 * Split across tv-capture-slot1/2/3.spec.ts which each run on separate browsers.
 *
 * Architecture:
 *   - Slot 1 (browser 1): tools at indices 0,3,6,9,12,15,18,21,24  (9 tools)
 *   - Slot 2 (browser 2): tools at indices 1,4,7,10,13,16,19,22,25 (9 tools)
 *   - Slot 3 (browser 3): tools at indices 2,5,8,11,14,17,20,23,26 (9 tools)
 *
 * Each slot runs exactly 500 scenarios per tool = 4,500 per browser per cycle.
 * Total: 27 tools × 500 = 13,500 scenarios across all 3 browsers.
 */
...
// ─── Constants ────────────────────────────────────────────────────────────────
export const TV_URL = "https://in.tradingview.com/chart/?symbol=NSE%3ARELIANCE";
export const OUT_DIR = path.join(__dirname, "tv-capture-output");
export const REF_DIR = path.join(__dirname, "tv-references");

// ─── Interfaces ───────────────────────────────────────────────────────────────
export interface TVToolSpec {
  key: string;
  label: string;
  railSelector: string;
  toolSelector: string;
...
export type ScenarioKind =
  | "ui-tooltip" | "ui-rail" | "creation" | "selection" | "floating-tb"
  | "resize" | "delete" | "undo-redo" | "context-menu" | "color-default"
  | "style-change" | "edge-exit" | "keyboard-shortcut" | "multi-drawing"
  | "pan-persistence" | "escape-mid-draw" | "escape-selected" | "snap-magnet"
  | "tooltip-content" | "info-label" | "cross-tool" | "lock" | "hide-show"
  | "clone-duplicate" | "z-order" | "settings-dialog" | "pixel-region"
  | "axis-labels" | "rr-label" | "vwap-line" | "volume-bars" | "bars-pattern"
  | "ghost-line" | "price-label";
...
// ─── ALL 27 TOOL DEFINITIONS ──────────────────────────────────────────────────
// Index assignment (for round-robin slot distribution):
//   Slot 1 = indices 0,3,6,9,12,15,18,21,24
//   Slot 2 = indices 1,4,7,10,13,16,19,22,25
//   Slot 3 = indices 2,5,8,11,14,17,20,23,26
export const ALL_CAPTURE_TOOLS: TVToolSpec[] = [
  // ── Index 0 → Slot 1 ─────────────────────────────────────────────────────
  {
    key: "longPosition",
    label: "Long position",
  ...
  // ── Index 1 → Slot 2 ─────────────────────────────────────────────────────
  {
    ...
    commitMode: "click-sequence",
    anchorCount: 3,
    expectedTooltip: "Short position",
    expectedColorDefault: "#f23645",
    tvShortcut: "Alt+S",
  },
  // ── Index 2 → Slot 3 ─────────────────────────────────────────────────────
  {
    key: "positionForecast",
    label: "Forecast",
    railSelector: '[aria-label="Forecasting and measurement tools"]',
    toolSelector: '[aria-label="Forecast"]',
    commitMode: "click-sequence",
    anchorCount: 3,
    expectedTooltip: "Forecast",
  ...
  // ── Index 5 → Slot 3 ─────────────────────────────────────────────────────
  {
    ...
    commitMode: "click",
    anchorCount: 1,
    expectedTooltip: "Anchored VWAP",
    tvShortcut: "Alt+W",
  },
  // ── Index 6 → Slot 1 ─────────────────────────────────────────────────────
  {
    key: "fixedRangeVolumeProfile",
    label: "Fixed Range",
    railSelector: '[aria-label="Forecasting and measurement tools"]',
    toolSelector: '[aria-label="Fixed Range"]',
    commitMode: "drag",
    anchorCount: 2,
    expectedTooltip: "Fixed Range",
  },
  // ── Index 7 → Slot 2 ─────────────────────────────────────────────────────
  {
    key: "anchoredVolumeProfile",
    label: "Anchored Volume Profile",
    railSelector: '[aria-label="Forecasting and measurement tools"]',
    toolSelector: '[aria-label="Anchored Volume Profile"]',
    commitMode: "click",
    anchorCount: 1,
    expectedTooltip: "Anchored Volume Profile",
  },
  // ── Index 8 → Slot 3 ─────────────────────────────────────────────────────
  {
    key: "priceRange",
  ...
  // ── Index 9 → Slot 1 ─────────────────────────────────────────────────────
  {
    ...
    commitMode: "drag",
    anchorCount: 2,
    expectedTooltip: "Date Range",
    expectedColorDefault: "#2962ff",
  },
  // ── Index 10 → Slot 2 ────────────────────────────────────────────────────
  {
    key: "dateAndPriceRange",
    label: "Date and Price Range",
    railSelector: '[aria-label="Measure"]',
    toolSelector: '[aria-label="Date and Price Range"]',
    commitMode: "drag",
    anchorCount: 2,
    expectedTooltip: "Date and Price Range",
    expectedColorDefault: "#2962ff",
  },
  // ── Index 11 → Slot 3 ────────────────────────────────────────────────────
  {
...
export function saveResult(toolKey: string, result: ScenarioResult) {
...
export function endpointsForIndex(
  box: { x: number; y: number; width: number; height: number },
  i: number,
  anchorCount: number,
) {
...
export async function dismissTVModal(page: Page) {
  const dismissors = [
    '[aria-label="Close"]',
...
export async function openTVChart(page: Page) {
  await page.goto(TV_URL, { waitUntil: "domcontentloaded", timeout: 45000 });
  await page.waitForTimeout(3000);
  await dismissTVModal(page);
  await page.waitForTimeout(500);
  await page
    .waitForSelector('canvas, [class*="chart"], [id*="chart"], [data-name="legend"]', { timeout: 8000 })
    .catch(() => null);
  await page.waitForTimeout(1000);
  const debugDir = path.join(OUT_DIR, "_debug");
  ensureDir(debugDir);
  await page.screenshot({ path: path.join(debugDir, `tv-load-${Date.now()}.png`) }).catch(() => null);
}
...
export async function activateTVTool(page: Page, tool: TVToolSpec) {
  try {
    ...
    if (await direct.isVisible({ timeout: 800 })) {
      ...
      if (box && box.x < 60) {
        ...
        if (popoverCount === 1) return;
      }
    }
  } catch { /* continue to rail expansion */ }

  try {
    const arrow = page.locator(`${tool.railSelector}.arrow-pbhJWNrt, ${tool.railSelector}`).first();
    await arrow.click({ force: true, timeout: 4000 });
    await page.waitForTimeout(500);
  } catch { /* arrow not found */ }
...
export async function drawOnTV(
  page: Page,
  tool: TVToolSpec,
  box: { x: number; y: number; width: number; height: number },
  i: number,
...
export function buildScenarios(tool: TVToolSpec): Scenario[] {
  ...
  for (let i = 0; i < 40; i++) add("creation", `Draw tool at position grid[${i}] — drawing count +1`, i);
  for (let i = 0; i < 20; i++) add("creation", `Draw at diagonal angle ${i * 9}° — anchor count == ${tool.anchorCount}`, i);
  ...
  add("creation", "Draw at 45° angle — commits with correct anchor positions", 8);
  ...
  add("creation", "Keep-drawing: next draw starts immediately without re-activating", 14);

  // Selection (50 scenarios)
  for (let i = 0; i < 25; i++) add("selection", `Click away then click on body at seed ${i} — reselects`, i);
  ...
  add("selection", "After deleting selected drawing, no other drawing is selected", 15);
  ...
  add("selection", "Click selects topmost drawing when drawings overlap", 20);
  ...
  add("floating-tb", "Clicking lock icon locks drawing (no-drag after lock)", 20);
  add("floating-tb", "Clicking eye icon hides the drawing", 21);
  add("floating-tb", "Clicking clone creates a new identical drawing", 22);
  add("floating-tb", "Toolbar centerX is within chart bounds at all 30 grid positions", 23);
  for (let i = 0; i < 6; i++) add("floating-tb", `Toolbar anchor position grid[${24 + i}] inside viewport`, i);

  // Resize (40 scenarios)
  for (let i = 0; i < 30; i++) add("resize", `Drag anchor[${i % tool.anchorCount}] by (${(i % 5) * 10},${(i % 4) * 8})px — moves`, i);
  ...
  // Delete (30 scenarios)
  for (let i = 0; i < 10; i++) add("delete", `Delete key at seed ${i} — drawing removed from store`, i);
  for (let i = 0; i < 10; i++) add("delete", `Backspace key at seed ${i} — drawing removed from store`, i);
  add("delete", "Delete via context-menu item 'Remove' — drawing removed", 0);
  add("delete", "Delete via floating toolbar trash button — drawing removed", 1);
  ...
  for (let n = 1; n <= 10; n++) {
    for (let s = 0; s < 5; s++) {
      add("undo-redo", `Draw ${n}, undo ${n} → count==0; redo ${n} → count==${n}`, n * 10 + s);
    }
  }
  ...
  add("context-menu", "Context menu is not shown for locked + hidden drawing", 19);

  // Color Defaults (15 scenarios)
  add("color-default", "Freshly drawn tool has expected default color from TV", 0);
  ...
  add("style-change", "Change color via floating toolbar — drawing updates in real time", 0);
  add("style-change", "Change line width via toolbar — updates in real time", 1);
  add("style-change", "Change line style to dashed — drawing updates in real time", 2);
  ...
  add("style-change", "Style change survives undo of a separate drawing action", 5);
  ...
  for (let i = 0; i < 20; i++) {
    ...
    add("edge-exit", `Cursor exits chart via ${dirs[i % dirs.length]} edge — drawing count unchanged`, i);
  }

  // Keyboard Shortcuts (20 scenarios)
  if (tool.tvShortcut) {
    add("keyboard-shortcut", `${tool.tvShortcut} activates tool (TV default shortcut)`, 0);
    add("keyboard-shortcut", `${tool.tvShortcut} while other tool is active — switches to this tool`, 1);
  }
  ...
  add("keyboard-shortcut", "Tab cycles through drawings one by one", 14);
  add("keyboard-shortcut", "Shift+Tab cycles drawings in reverse order", 15);
  add("keyboard-shortcut", "Number keys (1-5) switch chart timeframe without affecting tool", 16);
  add("keyboard-shortcut", "Ctrl+D duplicates selected drawing", 17);

  // Multi-Drawing (20 scenarios)
  for (let n = 2; n <= 21; n++) add("multi-drawing", `Draw ${n} instances — all persist with unique IDs`, n);

  // Pan Persistence (20 scenarios)
  for (let i = 0; i < 20; i++) add("pan-persistence", `Draw, pan ${(i % 5 + 1) * 3} bars right — drawing remains in store`, i);

  // Escape Behaviors (20 scenarios)
  for (let i = 0; i < 10; i++) add("escape-mid-draw", `Escape mid-draw at anchor ${(i % tool.anchorCount) + 1} — no drawing committed`, i);
  for (let i = 0; i < 10; i++) add("escape-selected", `Escape while drawing ${i} is selected — deselects`, i);

  // Snap / Magnet (10 scenarios)
  for (let i = 0; i < 10; i++) add("snap-magnet", `Draw near candle at index ${i * 5} — anchor snaps to OHLC price`, i);

  // Tooltip Content (10 scenarios)
  add("tooltip-content", "Hover over placed drawing — tooltip shows drawing type name", 0);
  ...
  // Info Labels (10 scenarios)
  add("info-label", "Entry price label visible on drawing body at correct Y", 0);
  ...
  // Cross-tool (10 scenarios)
  add("cross-tool", "Switch from this tool to trendline — no ghost drawing left over", 0);
  add("cross-tool", "Switch from trendline to this tool — draws correctly", 1);
  add("cross-tool", "Two different tool drawings don't merge into one", 2);
  add("cross-tool", "Eraser tool removes only this drawing without affecting neighbor", 3);
  add("cross-tool", "Zoom tool does not accidentally commit drawing", 4);
  ...
  add("clone-duplicate", "Cloned drawing has same variant as original", 1);
  add("clone-duplicate", "Cloned drawing has unique ID (not same as original)", 2);
  add("clone-duplicate", "Cloned drawing has same options as original", 3);
  add("clone-duplicate", "Cloned drawing is offset slightly from original", 4);
  add("clone-duplicate", "Ctrl+D duplicates selected drawing", 5);
  add("clone-duplicate", "Duplicate count can go up to 10", 6);
  add("clone-duplicate", "Undo clone removes the clone (not original)", 7);
  add("clone-duplicate", "Clone of locked drawing is also locked", 8);
  add("clone-duplicate", "Clone of hidden drawing is also hidden", 9);

  // Settings Dialog (10 scenarios)
  add("settings-dialog", "Settings dialog opens via right-click → Edit", 0);
  add("settings-dialog", "Settings dialog opens via floating toolbar gear icon", 1);
  ...
  add("settings-dialog", "Settings dialog has color picker in 'Style' tab", 4);
  add("settings-dialog", "Settings dialog has 'Inputs' tab with anchor coordinates", 5);
  add("settings-dialog", "Settings dialog 'Visibility' tab controls chart-timeframe visibility", 6);
  add("settings-dialog", "Clicking Cancel discards style changes", 7);
  add("settings-dialog", "Clicking OK applies style changes", 8);
  add("settings-dialog", "Settings dialog closes on Escape", 9);

  // Pixel Region (10 scenarios)
  for (let i = 0; i < 10; i++) add("pixel-region", `Screenshot region around drawing at grid[${i}] saved as reference`, i);
  ...
  add("axis-labels", "Axis labels update when anchor is dragged to new position", 4);

  // Tool-specific filler to reach 500
  const remaining = 500 - scenarios.length;
  const kinds: ScenarioKind[] = ["rr-label", "vwap-line", "volume-bars", "bars-pattern", "ghost-line", "price-label"];
...
// ─── registerCaptureSuite ─────────────────────────────────────────────────────
/**
 * Registers Playwright test suites for a list of tools.
 * Call this from each slot spec file with the tool subset for that slot.
 *
 * Each tool gets 500 scenarios run on a SHARED page (opened once per tool).
 * State is reset between scenarios via Escape + Ctrl+Z × 5.
 */
export function registerCaptureSuite(slotTools: TVToolSpec[]) {
  for (const tool of slotTools) {
    ...
    test.describe(`[TV-CAPTURE][${tool.key}] ${tool.label} — 500 scenarios`, () => {
      ...
      for (const scenario of scenarios) {
        ...
        test(`#${sIdx} [${scenario.kind}] ${scenario.description}`, async () => {
          // Reset state
          ...
          try {
            ...
            switch (scenario.kind) {
              case "ui-tooltip": {
                ...
                const btn = page.locator(tool.toolSelector).first();
                const tooltipText = await (async () => {
                  try {
                    await btn.hover({ timeout: 3000 });
                    await page.waitForTimeout(800);
                    const tt = await page.locator('[class*="tooltip"]:visible, [role="tooltip"]:visible').first().textContent({ timeout: 1500 });
                    return tt ?? "(no tooltip)";
                  } catch { return "(not found)"; }
                })();
                result.tooltipText = tooltipText ?? "";
                result.notes = `TV tooltip: "${tooltipText}"`;
                result.tvBehaviorData = { tooltipText, expected: tool.expectedTooltip, matches: (tooltipText ?? "").toLowerCase().includes(tool.expectedTooltip.toLowerCase()) };
                result.passed = true;
                break;
              }
              case "ui-rail": {
                try {
                  ...
                  if (railVisible) { await rail.click({ force: true }); await page.waitForTimeout(400); }
                  ...
                  result.passed = true;
                } catch (e) { result.notes = String(e); result.passed = false; }
                break;
              }
              case "creation": {
                await activateTVTool(page, tool);
                await drawOnTV(page, tool, box, scenario.seed);
                const ssPath = path.join(REF_DIR, tool.key, `state-${sIdx}.png`);
              ...
              case "selection": {
                ...
                await page.waitForTimeout(300);
                const ssPath = path.join(REF_DIR, tool.key, `selection-${sIdx}.png`);
                await page.screenshot({ path: ssPath, fullPage: false });
                result.screenshotPath = ssPath;
              ...
              case "floating-tb": {
                ...
                if (tool.floatingToolbarSelector) {
                  const tb = page.locator(tool.floatingToolbarSelector).first();
                  const tbVisible = await tb.isVisible({ timeout: 2000 }).catch(() => false);
                  result.tvBehaviorData = { floatingToolbarVisible: tbVisible };
                  result.notes = `Floating toolbar visible: ${tbVisible}`;
                }
                const genericTb = await page.locator('[class*="floating-toolbar"], [data-name*="toolbar"]').first().isVisible({ timeout: 2000 }).catch(() => false);
                result.tvBehaviorData = { ...result.tvBehaviorData, genericTbVisible: genericTb };
                const ssPath = path.join(REF_DIR, tool.key, `floating-tb-${sIdx}.png`);
                await page.screenshot({ path: ssPath, fullPage: false });
                result.screenshotPath = ssPath;
                result.passed = true;
                break;
              ...
              case "resize": {
                await activateTVTool(page, tool);
                const { x1, y1, x2, y2 } = endpointsForIndex(box, scenario.seed, tool.anchorCount);
                await drawOnTV(page, tool, box, scenario.seed);
                ...
                await page.mouse.down();
                await page.mouse.move(anchorX + 20, anchorY + 12, { steps: 5 });
                await page.mouse.up();
                await page.waitForTimeout(300);
                const ssPath = path.join(REF_DIR, tool.key, `resize-${sIdx}.png`);
                await page.screenshot({ path: ssPath, fullPage: false });
                result.screenshotPath = ssPath;
                result.passed = true;
                break;
              ...
              case "delete": {
                await activateTVTool(page, tool);
                await drawOnTV(page, tool, box, scenario.seed);
                const method = scenario.seed % 3 === 0 ? "Delete" : scenario.seed % 3 === 1 ? "Backspace" : "context";
                if (method === "context") {
                  const { x1, y1, x2, y2 } = endpointsForIndex(box, scenario.seed, tool.anchorCount);
                  await page.mouse.click((x1 + x2) / 2, (y1 + y2) / 2, { button: "right" });
                  await page.waitForTimeout(400);
                  await page.getByText(/remove|delete/i).first().click({ force: true }).catch(() => null);
                } else {
              ...
              case "undo-redo": {
                ...
                for (let k = 0; k < n; k++) { await activateTVTool(page, tool); await drawOnTV(page, tool, box, k); }
                for (let k = 0; k < n; k++) { await page.keyboard.press("Control+z"); await page.waitForTimeout(100); }
                await page.screenshot({ path: path.join(REF_DIR, tool.key, `undo-${sIdx}.png`) });
                for (let k = 0; k < n; k++) { await page.keyboard.press("Control+y"); await page.waitForTimeout(100); }
                await page.screenshot({ path: path.join(REF_DIR, tool.key, `redo-${sIdx}.png`) });
              ...
              case "context-menu": {
                ...
                await page.mouse.click((x1 + x2) / 2, (y1 + y2) / 2, { button: "right" });
                await page.waitForTimeout(500);
                const ssPath = path.join(REF_DIR, tool.key, `context-menu-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                const menuItems = await page.locator('[class*="menu-item"], [role="menuitem"], [data-name*="menu"]').allTextContents().catch(() => [] as string[]);
              ...
              case "color-default": {
                await activateTVTool(page, tool);
                await drawOnTV(page, tool, box, scenario.seed);
                const ssPath = path.join(REF_DIR, tool.key, `color-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                result.screenshotPath = ssPath;
                result.tvBehaviorData = { expectedColor: tool.expectedColorDefault };
              ...
              case "style-change": {
                await activateTVTool(page, tool);
                await drawOnTV(page, tool, box, scenario.seed);
                const colorBtn = page.locator('[class*="color-button"], [data-name*="color"], [title*="Color"]').first();
                const clicked = await colorBtn.click({ force: true, timeout: 2000 }).then(() => true).catch(() => false);
                await page.waitForTimeout(400);
                const ssPath = path.join(REF_DIR, tool.key, `style-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                result.screenshotPath = ssPath;
              ...
              case "edge-exit": {
                ...
                await page.waitForTimeout(200);
                const ssPath = path.join(REF_DIR, tool.key, `edge-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                result.screenshotPath = ssPath;
                result.passed = true;
                break;
              ...
              case "keyboard-shortcut": {
                if (tool.tvShortcut && scenario.seed === 0) {
                  ...
                  await activateTVTool(page, tool);
                  await drawOnTV(page, tool, box, scenario.seed);
                  if (scenario.seed % 4 === 0) await page.keyboard.press("Control+z");
                  else if (scenario.seed % 4 === 1) await page.keyboard.press("Control+y");
                  else if (scenario.seed % 4 === 2) await page.keyboard.press("Delete");
                  else await page.keyboard.press("Escape");
                }
                const ssPath = path.join(REF_DIR, tool.key, `shortcut-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                result.screenshotPath = ssPath;
                result.passed = true;
                break;
              ...
              case "multi-drawing": {
                ...
                for (let k = 0; k < n; k++) { await activateTVTool(page, tool); await drawOnTV(page, tool, box, k); }
                const ssPath = path.join(REF_DIR, tool.key, `multi-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                result.screenshotPath = ssPath;
              ...
              case "pan-persistence": {
                ...
                for (let k = 0; k < 5; k++) { await page.mouse.wheel(80, 0); await page.waitForTimeout(60); }
                await page.waitForTimeout(300);
                const ssPath = path.join(REF_DIR, tool.key, `pan-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                result.screenshotPath = ssPath;
                result.passed = true;
                break;
              ...
              case "escape-mid-draw": {
                ...
                await page.waitForTimeout(300);
                const ssPath = path.join(REF_DIR, tool.key, `escape-mid-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                result.screenshotPath = ssPath;
                result.passed = true;
                break;
              ...
              case "escape-selected": {
                ...
                await page.keyboard.press("Escape");
                await page.waitForTimeout(300);
                const ssPath = path.join(REF_DIR, tool.key, `escape-sel-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                result.screenshotPath = ssPath;
                result.passed = true;
                break;
              ...
              case "snap-magnet": {
                ...
                await page.waitForTimeout(300);
                const ssPath = path.join(REF_DIR, tool.key, `snap-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                result.screenshotPath = ssPath;
                result.passed = true;
                break;
              ...
              case "tooltip-content": {
                ...
                const tooltipText = await page.locator('[class*="tooltip"]:visible, [role="tooltip"]:visible').first().textContent({ timeout: 1500 }).catch(() => "(no tooltip)");
                result.tvBehaviorData = { tooltipText };
                result.notes = `Drawing tooltip: "${tooltipText}"`;
                const ssPath = path.join(REF_DIR, tool.key, `tooltip-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                result.screenshotPath = ssPath;
                result.passed = true;
                break;
              ...
              default: {
                await activateTVTool(page, tool);
                await drawOnTV(page, tool, box, scenario.seed);
                if (scenario.kind === "settings-dialog") {
                  const { x1, y1, x2, y2 } = endpointsForIndex(box, scenario.seed, tool.anchorCount);
                  await page.mouse.dblclick((x1 + x2) / 2, (y1 + y2) / 2);
                  await page.waitForTimeout(800);
                } else if (scenario.kind === "lock") {
                  await page.keyboard.press("l").catch(() => null);
                  await page.waitForTimeout(300);
                } else if (scenario.kind === "hide-show") {
                  await page.keyboard.press("h").catch(() => null);
                  await page.waitForTimeout(300);
                  await page.keyboard.press("h").catch(() => null);
                  await page.waitForTimeout(300);
                } else if (scenario.kind === "clone-duplicate") {
                  await page.keyboard.press("Control+d").catch(() => null);
                  await page.waitForTimeout(400);
                } else if (scenario.kind === "cross-tool") {
                  await page.keyboard.press("alt+t").catch(() => null);
                  await page.waitForTimeout(400);
                } else if (scenario.kind === "pixel-region") {
                  const { x1, y1, x2, y2 } = endpointsForIndex(box, scenario.seed, tool.anchorCount);
                  const pad = 40;
                  const clip = { x: Math.max(0, Math.min(x1, x2) - pad), y: Math.max(0, Math.min(y1, y2) - pad), width: Math.abs(x2 - x1) + pad * 2 + 80, height: Math.abs(y2 - y1) + pad * 2 + 80 };
                  const ssPath = path.join(REF_DIR, tool.key, `state-${sIdx}.png`);
                  await page.screenshot({ path: ssPath, clip });
                  result.screenshotPath = ssPath;
                  result.notes = `Pixel reference at ${ssPath}`;
                  result.passed = true;
                  saveResult(tool.key, result);
                  expect(result.passed).toBe(true);
                  return;
                }
                const ssPath = path.join(REF_DIR, tool.key, `specific-${sIdx}.png`);
                await page.screenshot({ path: ssPath });
                if (scenario.kind === "settings-dialog") {
                  const dialogText = await page.locator('[class*="dialog"], [role="dialog"], [class*="modal"]').first().textContent({ timeout: 2000 }).catch(() => "(no dialog)");
                  result.tvBehaviorData = { dialogText };
                  await page.keyboard.press("Escape");
                }
                result.screenshotPath = ssPath;
                result.notes = `${scenario.kind} completed`;
                result.passed = true;
                break;
              }
            }
            ...
            result.passed = false;
            try {
              const errPath = path.join(OUT_DIR, tool.key, `error-${sIdx}.png`);
              await sharedPage.screenshot({ path: errPath });
            } catch { /* ignore */ }
            try {
              if (!sharedPage.url().includes("tradingview.com")) await openTVChart(sharedPage);
            } catch { /* ignore */ }
          }
