/**
 * Dot cursor — 500 TradingView browser differentiation tests.
 * TV reference: dot/pin cursor (custom SVG URL, small dot shape).
 *
 * A (001-100) mode set/get round-trips
 * B (001-100) overlay CSS cursor contains url( (custom SVG dot)
 ...
 *
 * Run:
 *   npx playwright test e2e/cursor-dot-500.spec.ts \
...
const MODE: CursorMode = "dot";
const CSS_RE = /url\(/;
const OTHER: CursorMode[] = ["cross", "arrow", "demo", "eraser"];
...
test.describe(`[${MODE}] cursor — 500 TradingView parity tests`, () => {
  ...
  // ── B: overlay CSS cursor contains url( (custom SVG dot, 100 positions) ──
  for (let i = 0; i < 100; i++) {
