 ...
 *
 * Section A (FSA-A001â€“A100): Tool activation & configuration
 * Section B (FSA-B101â€“B200): Drawing creation (drag to draw)
 * Section C (FSA-C201â€“C300): Selection & keyboard interactions
 * Section D (FSA-D301â€“D400): Multi-drawing, undo/redo
 * Section E (FSA-E401â€“E500): Advanced behaviors & integrations
 *
 * Run:
...
const VARIANT = "fibSpeedResistArcs";
const TOOL_TEST_ID = "fib-speed-resistance-arcs";
const RAIL_TEST_ID = "rail-fib";

// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// SECTION A â€” Tool Activation & Configuration (FSA-A001â€“A100)
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

test.describe.serial("FSA Section A: Tool Activation (001-100)", () => {
  let page: Page;
  ...
  for (let i = 1; i <= 10; i++) {
    test(`FSA-A${String(i).padStart(3, "0")}: rail-fib button exists in ToolRail (run ${i})`, async () => {
      const rail = page.getByTestId(RAIL_TEST_ID);
      expect(await rail.count()).toBeGreaterThan(0);
    });
  }

  // A011-A020: clicking tool sets variant to fibSpeedResistArcs
  for (let i = 11; i <= 20; i++) {
    test(`FSA-A${String(i).padStart(3, "0")}: picking tool sets active variant to fibSpeedResistArcs (run ${i - 10})`, async () => {
  ...
  for (let i = 21; i <= 30; i++) {
    test(`FSA-A${String(i).padStart(3, "0")}: variant is fibSpeedResistArcs immediately after pick (run ${i - 20})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
      const v = await getActiveVariant(page);
      expect(v).toBe("fibSpeedResistArcs");
    });
  }

  // A031-A040: escape deactivates the tool
  for (let i = 31; i <= 40; i++) {
    test(`FSA-A${String(i).padStart(3, "0")}: Escape deactivates fibSpeedResistArcs (run ${i - 30})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
  ...
  for (let i = 41; i <= 50; i++) {
    test(`FSA-A${String(i).padStart(3, "0")}: chart-interaction-surface is visible when tool is active (run ${i - 40})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
  ...
  for (let i = 51; i <= 60; i++) {
    test(`FSA-A${String(i).padStart(3, "0")}: tool-rail element is visible (run ${i - 50})`, async () => {
  ...
  for (let i = 61; i <= 70; i++) {
    test(`FSA-A${String(i).padStart(3, "0")}: fib-retracement button has correct data-testid (run ${i - 60})`, async () => {
  ...
  // A071-A080: switching from fibSpeedResistArcs to fibExtension deactivates fibSpeedResistArcs
  for (let i = 71; i <= 80; i++) {
    test(`FSA-A${String(i).padStart(3, "0")}: switching to fibExtension changes variant away from fibSpeedResistArcs (run ${i - 70})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
      expect(await getActiveVariant(page)).toBe("fibSpeedResistArcs");
      await pickTool(page, "fib-extension");
      expect(await getActiveVariant(page)).not.toBe("fibSpeedResistArcs");
    });
  }

  // A081-A090: drawings count starts at 0 after reset
  for (let i = 81; i <= 90; i++) {
    test(`FSA-A${String(i).padStart(3, "0")}: drawings count is 0 after clearAll (run ${i - 80})`, async () => {
  ...
  for (let i = 91; i <= 100; i++) {
    test(`FSA-A${String(i).padStart(3, "0")}: no page error after picking fibSpeedResistArcs (run ${i - 90})`, async () => {
      const errors: string[] = [];
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// SECTION B â€” Drawing Creation (FSA-B101â€“B200)
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

test.describe.serial("FSA Section B: Drawing Creation (101-200)", () => {
  let page: Page;
  ...
  for (let i = 101; i <= 110; i++) {
    ...
    test(`FSA-B${i}: drag at position (${(pos.sx * 100).toFixed(0)}%, ${(pos.sy * 100).toFixed(0)}%) creates 1 drawing`, async () => {
      await drawFibTool(page, pos.sx, pos.sy, pos.ex, pos.ey);
      expect(await getDrawingsCount(page)).toBe(1);
    });
  }

  // B111-B120: drawing variant is fibSpeedResistArcs
  for (let i = 111; i <= 120; i++) {
    const pos = DRAW_POSITIONS[i - 111];
    test(`FSA-B${i}: created drawing has variant 'fibSpeedResistArcs' (position ${i - 110})`, async () => {
      await drawFibTool(page, pos.sx, pos.sy, pos.ex, pos.ey);
      const drawings = await getDrawings(page);
      expect(drawings.length).toBe(1);
      expect(drawings[0].variant).toBe("fibSpeedResistArcs");
    });
  }
  ...
  for (let i = 121; i <= 130; i++) {
    ...
    test(`FSA-B${i}: drawing has exactly 2 anchors (TV parity: 2-anchor tool) (pos ${i - 120})`, async () => {
      await drawFibTool(page, pos.sx, pos.sy, pos.ex, pos.ey);
      const drawings = await getDrawings(page);
  ...
  for (let i = 131; i <= 140; i++) {
    ...
    test(`FSA-B${i}: anchor[0] has valid time and price (pos ${i - 130})`, async () => {
      await drawFibTool(page, pos.sx, pos.sy, pos.ex, pos.ey);
      const drawings = await getDrawings(page);
  ...
  for (let i = 141; i <= 150; i++) {
    ...
    test(`FSA-B${i}: anchor[1] has valid time and price (pos ${i - 140})`, async () => {
      await drawFibTool(page, pos.sx, pos.sy, pos.ex, pos.ey);
      const drawings = await getDrawings(page);
  ...
  for (let i = 151; i <= 160; i++) {
    ...
    test(`FSA-B${i}: drawing family is 'fib' (TV parity) (pos ${i - 150})`, async () => {
      await drawFibTool(page, pos.sx, pos.sy, pos.ex, pos.ey);
      const drawings = await getDrawings(page);
  ...
  for (let i = 161; i <= 170; i++) {
    test(`FSA-B${i}: sequential draws increment count to ${i - 160}`, async () => {
      const n = i - 160;
      for (let j = 0; j < n; j++) {
  ...
  for (let i = 171; i <= 180; i++) {
    ...
    test(`FSA-B${i}: drawing has non-empty id string (pos ${i - 170})`, async () => {
      await drawFibTool(page, pos.sx, pos.sy, pos.ex, pos.ey);
      const drawings = await getDrawings(page);
  ...
  for (let i = 181; i <= 190; i++) {
    ...
    test(`FSA-B${i}: drawing.visible is true by default (TV parity) (pos ${i - 180})`, async () => {
      await drawFibTool(page, pos.sx, pos.sy, pos.ex, pos.ey);
      const drawings = await getDrawings(page);
  ...
  for (let i = 191; i <= 200; i++) {
    ...
    test(`FSA-B${i}: drawing.locked is false by default (TV parity) (pos ${i - 190})`, async () => {
      await drawFibTool(page, pos.sx, pos.sy, pos.ex, pos.ey);
      const drawings = await getDrawings(page);
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// SECTION C â€” Selection & Keyboard Interactions (FSA-C201â€“C300)
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

test.describe.serial("FSA Section C: Selection & Keyboard (201-300)", () => {
  let page: Page;
  ...
  for (let i = 201; i <= 210; i++) {
    test(`FSA-C${i}: forceSelectDrawing sets selectedDrawingId (run ${i - 200})`, async () => {
  ...
  for (let i = 211; i <= 220; i++) {
    test(`FSA-C${i}: Escape deselects selected fibSpeedResistArcs (TV parity) (run ${i - 210})`, async () => {
      await addSyntheticDrawings(page, 1, VARIANT);
      const drawings = await getDrawings(page);
      await forceSelectDrawing(page, drawings[0].id);
  ...
  for (let i = 221; i <= 230; i++) {
    test(`FSA-C${i}: Delete key removes selected fibSpeedResistArcs (TV parity) (run ${i - 220})`, async () => {
      await addSyntheticDrawings(page, 1, VARIANT);
      expect(await getDrawingsCount(page)).toBe(1);
      const drawings = await getDrawings(page);
      await forceSelectDrawing(page, drawings[0].id);
  ...
  for (let i = 231; i <= 240; i++) {
    test(`FSA-C${i}: Backspace removes selected fibSpeedResistArcs (TV parity) (run ${i - 230})`, async () => {
      await addSyntheticDrawings(page, 1, VARIANT);
      const drawings = await getDrawings(page);
      await forceSelectDrawing(page, drawings[0].id);
  ...
  for (let i = 241; i <= 250; i++) {
    test(`FSA-C${i}: drawing count is 0 after deleting only drawing (run ${i - 240})`, async () => {
      await addSyntheticDrawings(page, 1, VARIANT);
      const drawings = await getDrawings(page);
      await forceSelectDrawing(page, drawings[0].id);
      await page.keyboard.press("Delete");
  ...
  for (let i = 251; i <= 260; i++) {
    test(`FSA-C${i}: Delete removes only selected drawing, others remain (run ${i - 250})`, async () => {
  ...
  for (let i = 261; i <= 270; i++) {
    test(`FSA-C${i}: Ctrl+Z undoes drawing committed by drag (TV parity) (run ${i - 260})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
  ...
  for (let i = 271; i <= 280; i++) {
    test(`FSA-C${i}: Ctrl+Y redoes undone fibSpeedResistArcs (TV parity) (run ${i - 270})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
  ...
  for (let i = 281; i <= 290; i++) {
    test(`FSA-C${i}: Ctrl+Shift+Z redoes (TV alternate shortcut) (run ${i - 280})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
  ...
  for (let i = 291; i <= 300; i++) {
    ...
    test(`FSA-C${i}: ${drawCount} Ctrl+Z undoes bring count from ${drawCount} to 0`, async () => {
      await pickTool(page, TOOL_TEST_ID);
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// SECTION D â€” Multi-Drawing, Drag & Resize (FSA-D301â€“D400)
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

test.describe.serial("FSA Section D: Multi-Drawing & Drag (301-400)", () => {
  let page: Page;
  ...
  for (let i = 301; i <= 310; i++) {
    const n = i - 300; // 1-10
    test(`FSA-D${i}: drawing ${n} fibSpeedResistArcss results in count=${n} (TV parity)`, async () => {
      await addSyntheticDrawings(page, n, VARIANT);
      expect(await getDrawingsCount(page)).toBe(n);
    });
  }

  // D311-D320: all drawings have variant fibSpeedResistArcs
  for (let i = 311; i <= 320; i++) {
    const n = i - 310; // 1-10
    test(`FSA-D${i}: all ${n} drawings have variant='fibSpeedResistArcs' (TV parity)`, async () => {
      await addSyntheticDrawings(page, n, VARIANT);
      const drawings = await getDrawings(page);
  ...
  for (let i = 321; i <= 330; i++) {
    test(`FSA-D${i}: addSyntheticDrawings(10) creates 10 fibSpeedResistArcss (run ${i - 320})`, async () => {
  ...
  for (let i = 331; i <= 340; i++) {
    test(`FSA-D${i}: clearAll after 10 drawings leaves count=0 (run ${i - 330})`, async () => {
  ...
  for (let i = 341; i <= 350; i++) {
    test(`FSA-D${i}: undo/redo cycle preserves drawing state (run ${i - 340})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
  ...
  for (let i = 351; i <= 360; i++) {
    test(`FSA-D${i}: dragging selected drawing moves its anchors (TV parity) (run ${i - 350})`, async () => {
      await addSyntheticDrawings(page, 1, VARIANT);
  ...
  for (let i = 361; i <= 370; i++) {
    test(`FSA-D${i}: delete 1 of 5 fibSpeedResistArcss leaves 4 (TV parity) (run ${i - 360})`, async () => {
  ...
  // D371-D380: mix fibSpeedResistArcs with fibExtension, both exist
  for (let i = 371; i <= 380; i++) {
    test(`FSA-D${i}: fibSpeedResistArcs and fibExtension coexist in drawings list (run ${i - 370})`, async () => {
      await addSyntheticDrawings(page, 2, VARIANT);
      await addSyntheticDrawings(page, 2, "fibExtension");
      expect(await getDrawingsCount(page)).toBe(4);
      const drawings = await getDrawings(page);
      const hasRetracement = drawings.some((d) => d.variant === "fibSpeedResistArcs");
  ...
  // D381-D390: mix fibSpeedResistArcs with gannBox, both exist
  for (let i = 381; i <= 390; i++) {
    test(`FSA-D${i}: fibSpeedResistArcs and gannBox coexist in drawings list (run ${i - 380})`, async () => {
      await addSyntheticDrawings(page, 2, VARIANT);
      await addSyntheticDrawings(page, 2, "gannBox");
      expect(await getDrawingsCount(page)).toBe(4);
      const drawings = await getDrawings(page);
      expect(drawings.some((d) => d.variant === "fibSpeedResistArcs")).toBe(true);
  ...
  for (let i = 391; i <= 400; i++) {
    test(`FSA-D${i}: anchor[0].time != anchor[1].time in synthetic drawing (TV parity) (run ${i - 390})`, async () => {
      await addSyntheticDrawings(page, 1, VARIANT);
      const drawings = await getDrawings(page);
      const a0 = drawings[0].anchors[0];
      const a1 = drawings[0].anchors[1];
...
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•
// SECTION E â€” Advanced Behaviors & Integrations (FSA-E401â€“E500)
// â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•

test.describe.serial("FSA Section E: Advanced Behaviors (401-500)", () => {
  let page: Page;
  ...
  // E401-E410: draw 10 fibSpeedResistArcss, all have correct variant
  for (let i = 401; i <= 410; i++) {
    test(`FSA-E${i}: 10 synthetic fibSpeedResistArcss all have variant='fibSpeedResistArcs' (run ${i - 400})`, async () => {
      await addSyntheticDrawings(page, 10, VARIANT);
      const drawings = await getDrawings(page);
      expect(drawings.length).toBe(10);
      expect(drawings.every((d) => d.variant === "fibSpeedResistArcs")).toBe(true);
    });
  }

  // E411-E420: draw fibSpeedResistArcs via actual drag, then select it
  for (let i = 411; i <= 420; i++) {
    test(`FSA-E${i}: drawn fibSpeedResistArcs can be force-selected (run ${i - 410})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
  ...
  for (let i = 421; i <= 430; i++) {
    test(`FSA-E${i}: drawing count unchanged after scrolling chart (TV parity) (run ${i - 420})`, async () => {
      await addSyntheticDrawings(page, 3, VARIANT);
  ...
  for (let i = 431; i <= 440; i++) {
    test(`FSA-E${i}: drawing count unchanged after zoom (TV parity) (run ${i - 430})`, async () => {
      await addSyntheticDrawings(page, 2, VARIANT);
      const before = await getDrawingsCount(page);
      const box = await surfaceBox(page);
      await page.evaluate(({ x, y }) => {
  ...
  for (let i = 441; i <= 450; i++) {
    test(`FSA-E${i}: clearAll doesn't change active tool variant (TV parity) (run ${i - 440})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
      await addSyntheticDrawings(page, 5, VARIANT);
  ...
  // E451-E460: draw fibSpeedResistArcs + fibChannel, each has its own anchors
  for (let i = 451; i <= 460; i++) {
    test(`FSA-E${i}: fibSpeedResistArcs and fibChannel have independent anchor sets (run ${i - 450})`, async () => {
      await addSyntheticDrawings(page, 1, "fibSpeedResistArcs");
  ...
  for (let i = 461; i <= 470; i++) {
    test(`FSA-E${i}: delete gannBox leaves fibSpeedResistArcs intact (TV parity) (run ${i - 460})`, async () => {
      await addSyntheticDrawings(page, 1, "fibSpeedResistArcs");
      ...
      expect(remaining[0].variant).toBe("fibSpeedResistArcs");
    });
  }

  // E471-E480: drawing options object is populated
  for (let i = 471; i <= 480; i++) {
    test(`FSA-E${i}: drawing options object is non-null and has keys (TV parity) (run ${i - 470})`, async () => {
      await addSyntheticDrawings(page, 1, VARIANT);
      const drawings = await getDrawings(page);
  ...
  for (let i = 481; i <= 490; i++) {
    test(`FSA-E${i}: 3 draws then 3 undos returns to 0 drawings (TV parity) (run ${i - 480})`, async () => {
      await pickTool(page, TOOL_TEST_ID);
      for (let j = 0; j < 3; j++) {
  ...
  for (let i = 491; i <= 500; i++) {
    test(`FSA-E${i}: full draw-select-delete-undo lifecycle without crash (run ${i - 490})`, async () => {
      const errors: string[] = [];
      page.once("pageerror", (e) => errors.push(e.message));
