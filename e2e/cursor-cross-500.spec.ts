/**
 * Cross cursor — 500 TradingView browser differentiation tests.
 * TV reference: standard crosshair cursor (default chart mode).
 *
 * A (001-100) mode set/get round-trips
 * B (001-100) overlay CSS cursor === crosshair  (10×10 grid)
 ...
 *
 * Run:
 *   npx playwright test e2e/cursor-cross-500.spec.ts \
...
/** Clamp fractional coords to interior safe area of chart. */
function ib(
...
const MODE: CursorMode = "cross";
const CSS_RE = /crosshair/;
const OTHER: CursorMode[] = ["dot", "arrow", "demo", "eraser"];
...
test.describe(`[${MODE}] cursor — 500 TradingView parity tests`, () => {
  ...
  // ── B: overlay CSS cursor = crosshair (100 positions on 10×10 grid) ───────
  for (let i = 0; i < 100; i++) {
    const fx = 0.05 + (i % 10) * 0.09;
    const fy = 0.05 + Math.floor(i / 10) * 0.09;
    const label = `B${String(i + 1).padStart(3, "0")}`;
    test(
      `${label}. [${MODE}] cursor CSS at (${fx.toFixed(2)},${fy.toFixed(2)}) matches crosshair`,
      async () => {
    ...
    test(
      ...
      async () => {
        ...
        await plainDrag(
          page,
          ib(box, fx, fy),
          ib(box, Math.min(0.92, Math.max(0.04, fx + dx)), Math.min(0.92, Math.max(0.04, fy + dy))),
          7
        );
  ...
  // E001-E020 : N consecutive alt-drags → N strokes  (N = 1..5, ×4)
  for (let i = 0; i < 20; i++) {
    const n = (i % 5) + 1;
  ...
  for (let i = 0; i < 20; i++) {
    ...
    test(
      ...
      async () => {
        const r0 = await vr(page);
        const box = await chartBox(page);
        // drag right-to-left to reveal older bars (always available)
        await plainDrag(
          page,
