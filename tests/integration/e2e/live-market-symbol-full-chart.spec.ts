...
function buildQuote(symbol: string, index = 0) {
  return {
    symbol,
...
async function installChartFlowMocks(page: import("@playwright/test").Page): Promise<void> {
  ...
  await page.route("**/api/screener/symbol/**", async (route) => {
    const symbol = route.request().url().split("/").pop()?.toUpperCase() || "SPY";
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({
  ...
  await page.route("**/api/live/snapshot/public", async (route) => {
    ...
    const symbols = body?.symbols?.map((value) => value.toUpperCase()) ?? ["SPY"];
...
test("live market can open symbol page and fullscreen chart tools", async ({ page }) => {
  await installChartFlowMocks(page);
  await bootstrapAuthenticatedSession(page);

  await page.goto(`${baseUrl}/live-market`);
  await expect(page).toHaveURL(/\/live-market/);
  await expect(page.getByTestId("live-market-active-symbol")).toContainText("SPY");

  await page.getByTestId("live-market-open-symbol-page").click();
  ...
  await expect(page).toHaveURL(/\/simulation\?.*symbol=SPY/);
