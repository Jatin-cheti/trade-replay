...
/**
 * Loop 7: simplified usability gate.
 * Known-dead / region-blocked providers -> skip straight to colored initials.
 */
 ...
 * Loop 7: simplified usability gate.
 * Known-dead / region-blocked providers -> skip straight to colored initials.
 * No multi-candidate fallback chain; one failed <img> also collapses to initials.
 */
function isUsableSrc(src?: string | null): boolean {
  if (!src || typeof src !== "string") return false;
  if (src.startsWith("data:")) return true;
  if (!src.startsWith("http")) return false;
  if (src.includes("logo.clearbit.com")) return false;   // DNS dead
  if (src.includes("img.logo.dev")) return false;        // ORB / regional blocks
  if (src.includes("www.google.com/s2/favicons")) return false; // blocked/blank in many regions
  if (src.includes("icons.duckduckgo.com")) return false; // unreliable
  return true;
}

/**
 * Kept exported for backwards-compat with any existing srcset consumers / tests.
 * Loop 7: we no longer rely on CDN srcset variants -- returning "" disables srcset.
 */
function buildSrcSet(_src?: string): string {
  return "";
}
