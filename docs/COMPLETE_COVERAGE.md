...
# Manual Headed Verification Pass

**Date/time:** 2026-05-20, Asia/Kolkata
**TradingView URL used:** `https://in.tradingview.com/chart/?symbol=NSE%3ARELIANCE`
**Scope:** Phase 1 only — the 9 tools previously listed as activation-failed or factory-only.
**Evidence storage:** Screenshots and JSON are outside the repo at `C:\tmp\tv-headed-verification-2026-05-20\`; these files are evidence artifacts, not app/test changes.
**Method:** Headed Chromium via a temporary Playwright inspection script outside the repository. The TradingView Patterns rail was opened, the exact visible menu item was clicked, and the drawing was placed on the chart. The page was reloaded between tools to avoid cross-tool state.

Important caveat: a TradingView blue "Got it" education overlay appeared in some selected-state screenshots. It did not block activation or drawing placement, but exact floating-toolbar button interaction, settings-modal interaction, context-menu targeting, and handle dragging remain separate manual verification tasks.

## Phase 1 — Activation-Failed Tool Recheck

| Tool | Scenario | Old status | New status | Evidence type | Screenshot path | Notes |
|---|---|---|---|---|---|---|
| Cypher Pattern | Open Patterns > Chart patterns, activate `Cypher pattern`, place 5 anchors | activation-failed / factory-only | activation and 5-click creation verified | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-20\cypherPattern-01-menu.png`; `C:\tmp\tv-headed-verification-2026-05-20\cypherPattern-02-drawn-selected.png` | Exact menu item visible as `Cypher pattern`; drawing rendered with X/A/B/C/D labels and visible handles. Floating toolbar visible in screenshot but not fully interacted with. |
| Head and Shoulders | Open Patterns > Chart patterns, activate `Head and shoulders`, place 5 anchors | activation-failed / factory-only | activation and 5-click creation verified | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-20\headAndShoulders-01-menu.png`; `C:\tmp\tv-headed-verification-2026-05-20\headAndShoulders-02-drawn-selected.png` | Exact menu item visible as `Head and shoulders`; drawing placement completed. Selected/handle details need focused visual pass. |
| Triangle Pattern | Open Patterns > Chart patterns, activate `Triangle pattern`, place 3 anchors | activation-failed / factory-only | activation and 3-click creation verified | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-20\trianglePattern-01-menu.png`; `C:\tmp\tv-headed-verification-2026-05-20\trianglePattern-02-drawn-selected.png` | Exact menu item visible as `Triangle pattern`; drawing placement completed. |
| Three Drives Pattern | Open Patterns > Chart patterns, activate `Three drives pattern`, place 7 anchors | activation-failed / factory-only | activation and 7-click creation verified | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-20\threeDrives-01-menu.png`; `C:\tmp\tv-headed-verification-2026-05-20\threeDrives-02-drawn-selected.png` | Exact menu item visible as `Three drives pattern`; drawing placement completed. |
| Cyclic Lines | Open Patterns > Cycles, activate `Cyclic lines`, drag from first anchor to second anchor | activation-failed / factory-only | activation and drag creation verified | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-20\cyclicLines-01-menu.png`; `C:\tmp\tv-headed-verification-2026-05-20\cyclicLines-02-drawn-selected.png` | Exact menu item visible as `Cyclic lines`; creation is drag-based, not click-sequence. Screenshot shows repeated vertical cycle lines and two visible anchor handles. |
| Elliott Impulse | Open Patterns > Elliott waves, activate `Elliott impulse wave (1·2·3·4·5)`, place 5 anchors | activation-failed / factory-only | activation and 5-click creation verified | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-20\elliottImpulse-01-menu.png`; `C:\tmp\tv-headed-verification-2026-05-20\elliottImpulse-02-drawn-selected.png` | Exact current TradingView label uses middle dots: `1·2·3·4·5`. Drawing placement completed. |
| Elliott Correction | Open Patterns > Elliott waves, activate `Elliott correction wave (A·B·C)`, place 3 anchors | activation-failed / factory-only | activation and 3-click creation verified | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-20\elliottCorrection-01-menu.png`; `C:\tmp\tv-headed-verification-2026-05-20\elliottCorrection-02-drawn-selected.png` | Exact current TradingView label uses middle dots: `A·B·C`. Drawing placement completed. |
| Elliott Triangle | Open Patterns > Elliott waves, activate `Elliott triangle wave (A·B·C·D·E)`, place 5 anchors | activation-failed / factory-only | activation and 5-click creation verified | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-20\elliottTriangle-01-menu.png`; `C:\tmp\tv-headed-verification-2026-05-20\elliottTriangle-02-drawn-selected.png` | Exact current TradingView label uses middle dots: `A·B·C·D·E`. Drawing placement completed. |
| Elliott Double Combo | Open Patterns > Elliott waves, activate `Elliott double combo wave (W·X·Y)`, place 3 anchors | activation-failed / factory-only | activation and 3-click creation verified | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-20\elliottDoubleCombo-01-menu.png`; `C:\tmp\tv-headed-verification-2026-05-20\elliottDoubleCombo-02-drawn-selected.png` | Exact current TradingView label uses middle dots: `W·X·Y`. Drawing placement completed. |

## Phase 1 Status Changes

- Upgraded activation status for the 9 formerly activation-failed/factory-only tools to `COMPLETE_LIVE`.
- Upgraded creation-flow evidence for those 9 tools to `COMPLETE_LIVE`.
- Captured menu and drawn-state screenshots for each tool.
- Kept handle/white-dot, floating-toolbar controls, settings modal, context menu, tooltip/value reads, and crowded-chart behavior out of `COMPLETE_LIVE` unless separately verified.

## Phase 1 Remaining Blockers

- Floating toolbar exact controls remain `MANUAL_VERIFICATION_REQUIRED` for these 9 tools; screenshots prove visual presence, not dropdown/control behavior.
- Settings modal verification remains `MANUAL_VERIFICATION_REQUIRED`.
- Drawing-specific context menus remain `BLOCKED_CANVAS_HIT_TESTING` until right-click targets are proven.
- Handle positions are `SCREENSHOT_ONLY`; handle drag semantics still need `COMPLETE_LIVE` verification.
- Tooltip/value text is `SCREENSHOT_ONLY` where visible and otherwise `MANUAL_VERIFICATION_REQUIRED`.

## Phase 2 - Representative Toolbar / Settings / Context Recheck

**Date/time:** 2026-05-20, Asia/Kolkata
**TradingView URL used:** `https://in.tradingview.com/chart/?symbol=NSE%3ARELIANCE`
**Scope:** Representative manual/headed recheck for toolbar visibility, drawing-specific settings, drawing-specific context menus, selected handles, and annotation placement. This pass did not cover broad 59-tool automation, crowded-chart behavior, full toolbar dropdown interaction, or app implementation.
**Evidence storage:** Screenshots and JSON are outside the repo at `C:\tmp\tv-headed-verification-2026-05-20-reps\`; these files are evidence artifacts, not app/test changes.
**Method:** Headed Chromium via temporary Playwright inspection scripts outside the repository. Each representative tool was activated from the TradingView drawing menu, drawn on the chart, selected, and then probed for context menu and settings behavior where practical.

Important caveat: script-level context/settings classifications were reviewed against visible menu text. Menus containing chart/price items such as `Copy price`, `Buy 1 RELIANCE`, `Sell 1 RELIANCE`, or `Lock vertical cursor line by time` are treated as chart-level false positives, not drawing-specific context menus. Toolbar screenshots prove visual presence only; toolbar dropdowns/buttons were not fully opened and verified in this pass.

| Tool | Scenario | Old status | New status | Evidence type | Screenshot path | Notes |
|---|---|---|---|---|---|---|
| Trend Line | Draw/select, inspect floating toolbar, right-click body, open gear/settings | Toolbar heuristic; settings/context manual-required | Drawing-specific context menu and settings modal verified; toolbar/handles visual only | `COMPLETE_LIVE` for context/settings; `SCREENSHOT_ONLY` for toolbar/handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\trend-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\trend-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\trend-settings.png` | Context menu showed drawing items: `Add alert on trendline...`, `Template`, `Visual order`, `Visibility on intervals`, `Object tree`, `Clone`, `Copy`, `Lock`, `Hide`, `Remove`, `Settings...`. Settings modal title was `Trendline` with `Style`, `Text`, `Coordinates`, `Visibility`. |
| Info Line | Draw/select, inspect selected state, right-click body, open settings | Toolbar heuristic; settings/context manual-required | Drawing-specific context menu and settings modal verified; toolbar/handles visual only | `COMPLETE_LIVE` for context/settings; `SCREENSHOT_ONLY` for toolbar/handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\infoLine-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\infoLine-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\infoLine-settings.png` | Context menu showed `Add alert on info line...` plus standard drawing object actions. Settings opened from selected drawing; tooltip/value formatting still needs focused verification. |
| Trend Angle | Draw/select, inspect selected state, right-click body, open settings | Toolbar heuristic; settings/context manual-required | Drawing-specific context menu and settings modal verified; toolbar/handles visual only | `COMPLETE_LIVE` for context/settings; `SCREENSHOT_ONLY` for toolbar/handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\trendAngle-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\trendAngle-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\trendAngle-settings.png` | Context menu showed `Add alert on trend angle...` plus standard drawing object actions. Angle label/tooltip values remain visual/manual verification items. |
| Parallel Channel | Draw/select, right-click, open settings | Context/settings blocked or conflicting | Right-click/settings capture remained chart-level false positive | `BLOCKED_CANVAS_HIT_TESTING` for context; `CHART_SETTINGS_ONLY` for settings; `SCREENSHOT_ONLY` for selected handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\parallelChannel-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\parallelChannel-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\parallelChannel-settings.png` | Visible menu contained chart/price actions (`Copy price`, RELIANCE buy/sell items, vertical cursor lock). Do not count as drawing-specific context even though it included `Remove 1 drawing`. |
| Regression Trend | Draw/select, right-click, open settings | Context/settings blocked or conflicting | Right-click/settings capture remained chart-level false positive | `BLOCKED_CANVAS_HIT_TESTING` for context; `CHART_SETTINGS_ONLY` for settings; `SCREENSHOT_ONLY` for selected handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\regressionTrend-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\regressionTrend-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\regressionTrend-settings.png` | Context/settings targeting needs manual retry on stroke/handle/fill separately. |
| Disjoint Channel | Draw/select, right-click, open settings | Context/settings blocked or conflicting | Right-click/settings capture remained chart-level false positive | `BLOCKED_CANVAS_HIT_TESTING` for context; `CHART_SETTINGS_ONLY` for settings; `SCREENSHOT_ONLY` for selected handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\disjointChannel-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\disjointChannel-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\disjointChannel-settings.png` | Selected-state screenshot captured, but handle semantics still need live drag verification. |
| Pitchfork | Draw/select, right-click body, open settings | Toolbar heuristic; settings/context manual-required | Drawing-specific context menu and settings modal verified; toolbar/handles visual only | `COMPLETE_LIVE` for context/settings; `SCREENSHOT_ONLY` for toolbar/handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\pitchfork-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\pitchfork-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\pitchfork-settings.png` | Context menu included pitchfork conversion items: `Original`, `Schiff`, `Modified Schiff`, `Inside`, plus standard drawing actions. |
| Inside Pitchfork | Draw/select, right-click body, open settings | Toolbar heuristic; settings/context manual-required | Drawing-specific context menu and settings modal verified; toolbar/handles visual only | `COMPLETE_LIVE` for context/settings; `SCREENSHOT_ONLY` for toolbar/handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\insidePitchfork-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\insidePitchfork-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\insidePitchfork-settings.png` | Context menu included pitchfork conversion items and standard drawing actions. |
| Fib Retracement | Draw/select, right-click body, open settings | Toolbar heuristic; settings/context manual-required | Drawing-specific context menu and settings modal verified; toolbar/handles visual only | `COMPLETE_LIVE` for context/settings; `SCREENSHOT_ONLY` for toolbar/handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\fibRetracement-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\fibRetracement-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\fibRetracement-settings.png` | Context menu showed `Add alert on Fib retracement...` plus standard drawing actions. Level/label text remains screenshot/manual-only. |
| Fib Extension | Attempt activation from Fib menu labels | Conflicting/blocked settings and toolbar evidence | Activation not reached in this pass due current menu-label mismatch | `MANUAL_VERIFICATION_REQUIRED` | None captured in representative pass | Tried visible-label candidates including `Trend-based Fib Extension`, `Trend-Based Fib Extension`, `Fib extension`, and `Fib Extension`; current TradingView menu label/location needs manual retry. |
| Gann Box | Draw/select, right-click body, open settings | Toolbar heuristic; settings/context manual-required | Drawing-specific context menu observed; settings capture remained chart-level/conflicting | `COMPLETE_LIVE` for context; `CHART_SETTINGS_ONLY` / `CONFLICTING_REQUIRES_RETRY` for settings; `SCREENSHOT_ONLY` for handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\gannBox-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\gannBox-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\gannBox-settings.png` | Context menu contained standard drawing actions. Settings title/tabs need headed retry from the gear to confirm drawing-specific modal. |
| Pitchfan | Draw/select, right-click body, open settings | Toolbar heuristic; settings/context manual-required | Drawing-specific context menu and settings modal verified; toolbar/handles visual only | `COMPLETE_LIVE` for context/settings; `SCREENSHOT_ONLY` for toolbar/handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\pitchfan-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\pitchfan-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\pitchfan-settings.png` | Context menu showed standard drawing object actions. Fan level labels and handle semantics still need focused verification. |
| XABCD Pattern | Draw/select, right-click, open settings | Context/settings blocked or conflicting | Right-click/settings capture remained chart-level false positive | `BLOCKED_CANVAS_HIT_TESTING` for context; `CHART_SETTINGS_ONLY` for settings; `SCREENSHOT_ONLY` for selected handles/labels | `C:\tmp\tv-headed-verification-2026-05-20-reps\xabcd-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\xabcd-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\xabcd-settings.png` | Pattern labels/ratio text remain canvas/screenshot-only until manually read. |
| ABCD Pattern | Draw/select, right-click body, open settings | Toolbar heuristic; settings/context manual-required | Drawing-specific context menu and settings modal verified; toolbar/handles visual only | `COMPLETE_LIVE` for context/settings; `SCREENSHOT_ONLY` for toolbar/handles/labels | `C:\tmp\tv-headed-verification-2026-05-20-reps\abcdPattern-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\abcdPattern-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\abcdPattern-settings.png` | Context menu showed standard drawing object actions. Ratio labels still need exact visual/text verification. |
| Elliott Triple Combo | Draw/select, right-click, open settings | Context/settings blocked or conflicting | Right-click/settings capture remained chart-level false positive | `BLOCKED_CANVAS_HIT_TESTING` for context; `CHART_SETTINGS_ONLY` for settings; `SCREENSHOT_ONLY` for selected handles/labels | `C:\tmp\tv-headed-verification-2026-05-20-reps\elliottTripleCombo-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\elliottTripleCombo-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\elliottTripleCombo-settings.png` | Activation and drawing placement worked; context/settings need manual target retry. |
| Time Cycles | Draw/select, right-click, open settings | Context/settings blocked or conflicting | Right-click/settings capture remained chart-level false positive | `BLOCKED_CANVAS_HIT_TESTING` for context; `CHART_SETTINGS_ONLY` for settings; `SCREENSHOT_ONLY` for selected handles | `C:\tmp\tv-headed-verification-2026-05-20-reps\timeCycles-selected.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\timeCycles-context.png`; `C:\tmp\tv-headed-verification-2026-05-20-reps\timeCycles-settings.png` | Cycle spacing/handle drag behavior remains manual-required. |
| Text | Activate `Text`, place on chart | Text/annotation behavior manual-required | Activation/placement screenshot captured only | `COMPLETE_LIVE` for activation; `SCREENSHOT_ONLY` for placement; `MANUAL_VERIFICATION_REQUIRED` for text editing/movement/settings/context | `C:\tmp\tv-headed-verification-2026-05-20-reps\plainText-placed-or-prompt.png` | Text entry/editing, attached movement, persistence, settings, and context menu were not completed in this pass. |
| Callout | Activate `Callout`, place on chart | Text/annotation behavior manual-required | Activation/placement screenshot captured only | `COMPLETE_LIVE` for activation; `SCREENSHOT_ONLY` for placement; `MANUAL_VERIFICATION_REQUIRED` for text editing/movement/settings/context | `C:\tmp\tv-headed-verification-2026-05-20-reps\callout-placed-or-prompt.png` | Callout body/anchor/text semantics still need focused headed verification. |
| Price Label | Activate `Price label`, place on chart | Text/annotation behavior manual-required | Activation/placement screenshot captured only | `COMPLETE_LIVE` for activation; `SCREENSHOT_ONLY` for placement; `MANUAL_VERIFICATION_REQUIRED` for text editing/movement/settings/context | `C:\tmp\tv-headed-verification-2026-05-20-reps\priceLabel-placed-or-prompt.png` | Price attachment and label persistence across zoom/pan still need verification. |
| Image | Activate `Image`, place on chart | Text/annotation/media behavior manual-required | Activation/prompt screenshot captured only | `COMPLETE_LIVE` for activation; `SCREENSHOT_ONLY` for prompt/placement; `MANUAL_VERIFICATION_REQUIRED` for upload/media/settings/context | `C:\tmp\tv-headed-verification-2026-05-20-reps\image-placed-or-prompt.png` | Media picker/upload behavior was not completed. Do not treat this as image tool parity. |

## Phase 2 Status Changes

- Upgraded drawing-specific context menu evidence to `COMPLETE_LIVE` for representative tools where the actual drawing menu was observed: Trend Line, Info Line, Trend Angle, Pitchfork, Inside Pitchfork, Fib Retracement, Gann Box, Pitchfan, and ABCD Pattern.
- Upgraded drawing-specific settings evidence to `COMPLETE_LIVE` for Trend Line, Info Line, Trend Angle, Pitchfork, Inside Pitchfork, Fib Retracement, Pitchfan, and ABCD Pattern.
- Kept Parallel Channel, Regression Trend, Disjoint Channel, XABCD Pattern, Elliott Triple Combo, and Time Cycles context menus at `BLOCKED_CANVAS_HIT_TESTING` because right-click produced chart/price-level menus.
- Kept Parallel Channel, Regression Trend, Disjoint Channel, XABCD Pattern, Elliott Triple Combo, and Time Cycles settings at `CHART_SETTINGS_ONLY` because the captured settings did not prove a drawing-specific modal.
- Marked Gann Box settings as `CHART_SETTINGS_ONLY` / `CONFLICTING_REQUIRES_RETRY` because context evidence was drawing-specific but the settings capture still needs title/tab confirmation.
- Kept toolbar exact controls, dropdowns, handle drag semantics, tooltip/value reads, text-label movement, zoom/pan/offscreen behavior, and crowded-chart behavior out of `COMPLETE_LIVE`.

## Phase 2 Remaining Blockers

- Floating toolbar exact buttons/dropdowns remain `MANUAL_VERIFICATION_REQUIRED`; selected screenshots are only `SCREENSHOT_ONLY` evidence.
- Context menu targeting still needs manual headed verification for every family not explicitly upgraded above, and for all target regions: body/stroke, endpoint/anchor, label/text, fill/background, and metric box.
- Settings modal verification still needs retries for chart-level false positives and conflicting title/tabs, especially channels, XABCD, Elliott Triple Combo, Time Cycles, Gann Box, and Fib Extension.
- Handle/white-dot counts and roles are visible only as `SCREENSHOT_ONLY`; each handle type still needs drag verification.
- Text/annotation tools were activated, but editing, movement, deletion, persistence, and settings/context behavior remain `MANUAL_VERIFICATION_REQUIRED`.
- Crowded-chart behavior was not tested in this pass and remains `MANUAL_VERIFICATION_REQUIRED`.

# Manual Headed Verification Pass 2 — Toolbar, Handles, Text, Tooltip, Viewport

**Date/time:** 2026-05-21, Asia/Kolkata
**TradingView URL used:** `https://in.tradingview.com/chart/?symbol=NSE%3ARELIANCE`
**Scope:** Focused representative verification for exact floating-toolbar controls, dropdown/picker behavior, handle/white-dot roles, text editing/movement, tooltip/value reads, Fib Extension menu retry, viewport behavior, and a small crowded-chart smoke. This pass did not attempt full 59-tool parity and did not modify app code, tests, specs, or factories.
**Evidence storage:** Screenshots and JSON are outside the repo at `C:\tmp\tv-headed-verification-2026-05-21-pass2\`; these files are evidence artifacts, not app/test changes.
**Method:** Headed Chromium via a temporary Playwright inspection script outside the repository. DOM data was used only when it exposed selected drawing toolbar `data-name` attributes or readable menu/modal text. Canvas-rendered drawings, handles, labels, and tooltip values remain screenshot/manual evidence unless explicitly DOM-readable.

Important caveat: the attempt to change Trend Line width from the dropdown accidentally focused a watchlist item in a later screenshot sequence. The width menu itself is valid evidence, but the subsequent viewport/crowded screenshots after the symbol switch are treated as partial or retry-required rather than complete viewport proof.

| Tool | Scenario | Old status | New status | Evidence label | Screenshot path | Observed behavior | Remaining blocker |
|---|---|---|---|---|---|---|---|
| Trend Line | Selected floating toolbar inventory | Toolbar buttons were heuristic-only or screenshot-only | Toolbar control names and positions extracted from live selected drawing DOM | `COMPLETE_DOM` for control inventory; `SCREENSHOT_ONLY` for visual toolbar | `C:\tmp\tv-headed-verification-2026-05-21-pass2\pass2-trend-selected.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\pass2-results.json` | The selected drawing toolbar exposed `data-name="drawing-toolbar"` with controls: `templates`, `line-tool-color`, `text-color`, `line-tool-width` showing default `2px`, `style`, `settings`, `add-alert`, `lock`, `remove`, and `more`. | Per-control functional effects, lock/hide/delete/copy safety checks, and toolbar persistence after every drag/zoom/pan scenario still need retry. |
| Trend Line | Line width dropdown | Exact dropdown options were manual-required | Width dropdown opened and displayed concrete options | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-21-pass2\trend-toolbar-line-tool-width.png` | Dropdown showed `1px`, `2px`, `3px`, `4px`, with `2px` selected by default. | Changing an option needs a clean retry because the later option-click attempt caused an unrelated watchlist focus. |
| Trend Line | Style dropdown | Exact style options were manual-required | Style dropdown opened and displayed concrete options | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-21-pass2\trend-toolbar-style.png` | Dropdown showed `Line`, `Dashed line`, and `Dotted line`, with `Line` selected by default. | Need safe option-change verification and confirmation that the drawn stroke updates immediately. |
| Trend Line | Stroke/text color pickers | Color picker was heuristic/source-only | Stroke and text color pickers opened as palettes | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-21-pass2\trend-toolbar-line-tool-color.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\trend-toolbar-text-color.png` | The picker displayed swatch grid, plus custom color add control and opacity slider. Stroke picker showed current blue selection. | Need safe color/opacity change verification and Escape/click-outside close behavior per picker. |
| Trend Line | More menu | More/options menu was manual-required | More menu opened and showed object actions | `COMPLETE_LIVE` + `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-21-pass2\trend-toolbar-more.png` | Menu showed `Visual order`, `Visibility on intervals`, `Clone`, `Copy`, and `Hide`. | Visual order submenu, visibility submenu, clone/copy side effects, and hide/restore behavior need clean retries. |
| Trend Line | Endpoint handles and drag semantics | Handles were screenshot-only | Two circular endpoint handles observed; body/endpoint drag attempted | `COMPLETE_LIVE` for attempted drag actions; `SCREENSHOT_ONLY` for exact handle roles/positions | `C:\tmp\tv-headed-verification-2026-05-21-pass2\pass2-trend-selected.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\trend-after-body-drag.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\trend-after-endpoint-drag.png` | Selected Trend Line showed two blue/white circular endpoint handles. Endpoint drag screenshot showed a single endpoint edit with dashed guide/angle helper; body drag was attempted but not measured. | Need pixel-position measurement, cursor capture, and clean before/after coordinate comparison for body drag and each endpoint. |
| Info Line | Drawing tooltip/value read while drawing | Tooltip/value formatting was manual-required | Preview tooltip captured visually while moving before final click | `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-21-pass2\infoLine-preview-tooltip.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\infoLine-selected-pass2.png` | Canvas tooltip showed rows including price change, percent change, bar count/days, distance in px, and angle. Example visible values included `74.75 (5.24%), 1,495`, `45 bars (68d), distance: 314 px`, and `30.65°`. | DOM extraction was not reliable; exact formatting needs manual transcription per viewport/scale and dynamic update verification while dragging. |
| Trend Angle | Angle helper/value read while drawing | Angle tooltip/value formatting was manual-required | Preview angle helper captured visually | `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-21-pass2\trendAngle-preview-tooltip.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\trendAngle-selected-pass2.png` | Canvas overlay showed an angle label near the line and dashed guide geometry during creation. | Exact angle text is screenshot-only; dynamic update while dragging needs focused manual read. |
| Fib Extension | Current menu-label/location retry | Activation previously failed because candidate labels did not match | Current TradingView label/location confirmed and drawing created | `COMPLETE_LIVE` for activation/creation; `SCREENSHOT_ONLY` for handles/levels/labels | `C:\tmp\tv-headed-verification-2026-05-21-pass2\fib-extension-menu-current-label.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\fibExtension-drawn-selected-pass2.png` | Current menu location is Gann and Fibonacci tools > Fibonacci > `Trend-based fib extension`. Creation used 3 anchors. Selected drawing showed extension level labels including `0`, `0.236`, `0.382`, `0.5`, `0.618`, `0.786`, `1`, `1.618`, `2.618`, and `3.618`, with the floating toolbar visible. | Exact handle roles, level settings, label formatting, tooltip reads, and settings modal fields still need verification. |
| Text | Text creation/edit/move smoke | Text/annotation behavior was manual-required | Text tool activated, text was typed, selected, dragged, and undo attempted | `COMPLETE_LIVE` for basic create/type/drag attempt; `SCREENSHOT_ONLY` for attachment behavior | `C:\tmp\tv-headed-verification-2026-05-21-pass2\text-after-type-pass2.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\text-selected-pass2.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\text-after-drag-pass2.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\text-after-undo-pass2.png` | Text `TV text attach test` appeared as an editable selected text object with toolbar controls. After drag attempt, the text remained attached to the chart coordinates and selection/toolbar state changed. | Separate text edit, delete, copy/clone, undo/redo semantics, click-text selection behavior, and persistence after zoom/pan/deselect/reselect need a clean manual pass. |
| Price Range | Metric tooltip/value preview | Tooltip/value formatting was manual-required | Metric label captured during creation | `SCREENSHOT_ONLY` | `C:\tmp\tv-headed-verification-2026-05-21-pass2\priceRange-preview-tooltip.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\priceRange-selected-pass2.png` | Preview label showed price/percent/bar metric text, example visible text `93.45 (6.63%) 1,869`. | Need exact DOM/canvas determination, dynamic update while dragging, and settings/context verification. |
| Trend Line | Viewport and crowded-chart smoke | Crowded/viewport behavior remained manual-required | Small smoke attempted but not upgraded because later artifacts were contaminated by watchlist/symbol focus | `CONFLICTING_REQUIRES_RETRY` | `C:\tmp\tv-headed-verification-2026-05-21-pass2\trend-after-zoom-in.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\trend-after-zoom-out.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\trend-after-pan.png`; `C:\tmp\tv-headed-verification-2026-05-21-pass2\crowded-trendlines-12-selected-last.png` | Zoom/pan and 12 same-type drawing smoke were attempted, but the earlier unsafe width-option click changed chart context to a watchlist symbol. Do not treat this as complete viewport/crowded evidence. | Retry from a clean chart with no watchlist focus; target 30-40 same-type drawings and 30-40 mixed drawings as requested. |

## Pass 2 Status Changes

- Upgraded Trend Line floating-toolbar control inventory to `COMPLETE_DOM` for the exact selected-toolbar control names exposed by TradingView.
- Upgraded Trend Line width/style/color/more dropdown opening to `COMPLETE_LIVE` + `SCREENSHOT_ONLY` for visible options and palettes.
- Upgraded Fib Extension activation/creation from retry-required to `COMPLETE_LIVE` with current menu label `Trend-based fib extension`.
- Added `SCREENSHOT_ONLY` tooltip/value evidence for Info Line, Trend Angle, and Price Range.
- Added first-pass Text creation/typing/drag evidence, but kept full text/label attachment semantics manual-required.

## Pass 2 Remaining Blockers

- Toolbar button side effects still need clean verification: lock, hide, delete, clone/copy, add alert, templates, settings, Escape close, click-outside close, and persistence after drag/zoom/pan/deselect/reselect.
- Handle/white-dot behavior still needs exact dot counts, pixel positions, hover-only states, cursor capture, and per-handle before/after coordinate measurement.
- Text/annotation tools still need separate editing, deletion, copy/clone, undo/redo, click-target, and zoom/pan persistence verification.
- Tooltip/value reads for Info Line, Trend Angle, Fib tools, Long/Short Position, Price Range, and Date Range remain mostly `SCREENSHOT_ONLY` because TradingView renders the values on canvas.
