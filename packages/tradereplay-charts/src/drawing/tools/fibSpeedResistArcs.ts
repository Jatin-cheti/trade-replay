/**
 * Fib Speed Resistance Arcs — TradingView parity.
 * Arcs centered at anchor[0] with radii at fib multiples of |a-b|, rendered
 * as semicircles in the half-plane facing anchor[1]. Anchor count: 2.
 */
...
export class FibSpeedResistArcsTool extends FibBaseTool {
  readonly variant: DrawingVariant = 'fibSpeedResistArcs';
  readonly label = 'Fib Speed Resistance Arcs';
  ...
  override render(
    ...
    for (const lv of levels) {
      ...
      ctx.beginPath();
      ctx.arc(a.x, a.y, r * lv, angle - Math.PI / 2, angle + Math.PI / 2);
      ctx.stroke();
    }
    ctx.setLineDash([4, 4]);
  ...
  override getAxisHighlight(drawing: Drawing, viewport: Viewport): AxisHighlight | null {
    ...
    const b = dataToScreen(drawing.anchors[1], viewport);
    const r = distancePx(a, b);
    return { xRange: [a.x - r, a.x + r], yRange: [a.y - r, a.y + r] };
  }
}
